...
def _detect_screen_spoof(filepath: str) -> bool:
    """
    Balanced screen-spoof detection using 2D Fast Fourier Transform (FFT) 
    with adjusted thresholds to prevent false positives on real outdoor road textures.
    """
    try:
        ...
        # Balanced thresholds to allow real grainy/asphalt textures while catching true monitors
        if max_peak > 180 or mean_high > 45:
            return True
    except Exception as e:
        print(f"Screen detection check error: {e}")
    return False
...
def _validate_image_quality(filepath: str) -> Optional[str]:
    """Validates minimum resolution, brightness, and blur using OpenCV with adjusted mobile tolerances."""
    try:
        ...
        h, w = img.shape[:2]
        # Adjusted Minimum Resolution Check for mobile devices (640x480)
        if w < 640 or h < 480:
            return f"Resolution too low ({w}x{h}). Minimum required is 640x480 pixels."

        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

        # Adjusted Brightness / Dim Check for night testing
        avg_brightness = float(gray.mean())  # type: ignore
        if avg_brightness < 20.0:
            return "Photo is too dim or dark. Please use better lighting or flash."

        # Adjusted Blur Detection (Laplacian Variance) for handheld phone cameras
...
async def create_complaint(
    ...
    # BYPASSED FOR TESTING: Screen spoof check disabled to prevent false positives on paver blocks/tiles
    ...
    # 2. Camera Quality Validation with mobile-friendly tolerances
